...
st.title("🚛 Roteirizador Inteligente com Redução de Custos")

# 📌 Entradas
api_key = st.text_input("🔑 API Key do OpenRouteService", type="password")
...
custo_km = st.number_input("💸 Custo por km (R$)", value=2.5)

# 🎯 Funções auxiliares

def consultar_chuva(lat, lon):
    """Consulta se está chovendo em uma coordenada usando a API pública do Open-Meteo."""
...
def calcular_risco(lat, lon, chovendo):
    """Simula risco baseado em localização e chuva real."""
    hotspots = [
        (-23.555, -46.635),  # roubo
...
def calcular_score_final(dist_km, tempo_min, risco):
    ...
    penalidade_trânsito = tempo_min * 0.1
    return base + penalidade_risco + penalidade_trânsito

# 🔄 Execução principal
if st.button("🚀 Calcular Rota Ideal"):
    if not api_key:
        st.warning("⚠️ Insira uma chave válida do OpenRouteService.")
    else:
        try:
            ...
            st.markdown(f"""
            ### 📊 Resultados da Rota
            - 🛣️ **Distância:** {dist_km:.2f} km
            - ⏱️ **Tempo estimado:** {duracao_min:.1f} min
            - 🌧️ **Chuva detectada:** {"Sim" if any(consultar_chuva(lat, lon) for lon, lat in geo['coordinates']) else "Não"}
            - ⚠️ **Risco médio da rota:** {risco_medio:.2f}
